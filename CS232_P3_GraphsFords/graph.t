...
*   Function Name     :		mst
*
*   Purpose           :		constructs a minimum spanning tree
*
*   Input Parameters  :		none
*
*	Output parameters  :	none
...
{	
	...
	int counter = adj_list.size();
	cout << "Minimum Spanning Tree" << endl;
	cout << "Start point is: " << adj_list[target].name << endl;

	//copies names of verticies and creates the MST
	for (int i = 0; i < adj_list.size(); i++) {
		tempMSTRep.name = adj_list[i].name;
		MST.push_back(tempMSTRep);
	}

	while (counter != 0) { //or target !=counter
		for (int j = 0; j < adj_list[target].edgelist.size(); j++) {
			MST[target].component = true;
			if (adj_list[target].edgelist[j].weight < MST[target].distance) {
				MST[target].neighbor = adj_list[target].edgelist[j].name;
				MST[target].distance = adj_list[target].edgelist[j].weight;
			}
		}
		for (int k = 0; k < MST.size(); k++) {//iterates MST to print
			cout << MST[k].name << " " << MST[k].component << " " << MST[k].distance << " "
				<< MST[k].neighbor << endl;
		}
		cout << endl;
		counter--;
	...
	for (int k = 0; k < MST.size(); k++) {//iterates MST to print
		if (MST[k].distance != 999999) {
			total = MST[k].distance+total;
		}
	}
	cout << "MST of graph is: " << total << endl;
}
///END MST///
